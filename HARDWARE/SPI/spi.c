...
void WIZ_CS(uint8_t val)
{
	if (val == LOW) 
	{
   		HAL_GPIO_WritePin(GPIOE, WIZ_SCS,GPIO_PIN_RESET);	// GPIOB的WIZ_SCS（GPIO_Pin_12）引脚清零拉低
	}
	else if (val == HIGH)
	{
   		HAL_GPIO_WritePin(GPIOE, WIZ_SCS,GPIO_PIN_SET); 	// GPIOB的WIZ_SCS（GPIO_Pin_12）引脚置1拉高
	}
}
